  ...
  return {
    ...
    runAction: async (inputs?: Partial<ActionConfig>) => {
      ...
      config = {
        ...config,
        ...{
          versionFormat: '${major}.${database}.${minor}.${patch}+${increment}',
        },
        ...repoDefaultConfig,
        ...inputs,
      }
...
test('Empty repository version is correct', async () => {
  ...
  expect(result.formattedVersion).toBe('0.0.0.0+0')
}, 15000)
...
test('Repository with commits shows increment', async () => {
  ...
  expect(result.formattedVersion).toBe('0.0.0.1+1')
}, 15000)
...
test('Repository show commit for checked out commit', async () => {
  ...
  expect(result.formattedVersion).toBe('0.0.0.1+0')
}, 15000)
...
test('Tagging does not break version', async () => {
  ...
  expect(result.formattedVersion).toBe('0.0.0.1+2')
}, 15000)
...
test('Minor update bumps minor version and resets increment', async () => {
  ...
  expect(result.formattedVersion).toBe('0.0.1.0+0')
}, 15000)
...
test('Major update bumps major version and resets increment', async () => {
  ...
  expect(result.formattedVersion).toBe('1.0.0.0+0')
}, 15000)
...
test('Multiple major commits are idempotent', async () => {
  ...
  expect(result.formattedVersion).toBe('1.0.0.0+1')
}, 15000)
...
test('Tags start new version', async () => {
  ...
  repo.makeCommit('Initial Commit') // 0.0.1+0
  repo.makeCommit('Second Commit') // 0.0.1+1
  repo.exec('git tag v0.0.0.1')
  repo.makeCommit('Third Commit') // 0.0.2+0
  const result = await repo.runAction()
  console.log(result)
  expect(result.formattedVersion).toBe('0.0.0.2+0')
}, 15000)
...
test('Version pulled from last release branch', async () => {
  ...
  repo.exec('git tag v5.0.6.7')
  repo.makeCommit('Third Commit') // 5.6.7+0
  const result = await repo.runAction()

  expect(result.formattedVersion).toBe('5.0.6.8+0')
}, 15000)
...
test('Merged tags do not affect version', async () => {
  ...
  repo.makeCommit('Second Commit') // 0.0.1+1
  repo.makeCommit('Third Commit') // 0.1.1+2
  repo.exec('git checkout -b release/0.0.0.1')
  repo.makeCommit('Fourth Commit') // 0.1.1+3
  repo.exec('git tag v0.0.0.1')
  repo.exec('git checkout master')
  repo.makeCommit('Fifth Commit') // 0.0.2.0
  repo.exec('git tag v0.0.0.2')
  repo.exec('git merge release/0.0.0.1')
  const result = await repo.runAction()

  expect(result.formattedVersion).toBe('0.0.0.3+1')
}, 15000)

test('Format input is respected', async () => {
  const repo = createTestRepo({
    versionFormat: 'M${major}d${database}m${minor}p${patch}i${increment}',
  }) // M0m0p0i0

  repo.makeCommit('Initial Commit') // M1m2p3i0
  repo.exec('git tag v1.0.2.3')
  repo.makeCommit(`Second Commit`) // M1m2p4i0
  const result = await repo.runAction()

  expect(result.formattedVersion).toBe('M1d0m2p4i0')
}, 15000)
...
test('Namespace is tracked separately', async () => {
  ...
  repo.exec('git tag 0.0.0.1')
  repo.makeCommit('Second Commit') // 0.0.2
  repo.exec('git tag 0.0.1.0-subproject')
  repo.makeCommit('Third Commit') // 0.0.2 / 0.1.1

  const result = await repo.runAction()
  const subprojectResult = await repo.runAction({ namespace: 'subproject' })

  expect(result.formattedVersion).toBe('0.0.0.2+1')
  expect(subprojectResult.formattedVersion).toBe('0.0.1.1+0')
}, 15000)
...
test('Namespace allows dashes', async () => {
  ...
  repo.exec('git tag 0.0.0.1')
  repo.makeCommit('Second Commit') // 0.0.2
  repo.exec('git tag "0.0.1.0-sub/project"')
  repo.makeCommit('Third Commit') // 0.0.2 / 0.1.1
...
test('Current tag is used', async () => {
  ...
  repo.makeCommit('Second Commit')
  repo.makeCommit('Third Commit')
  repo.exec('git tag 7.0.6.5')

  const result = await repo.runAction()

  expect(result.formattedVersion).toBe('7.0.6.5+0')
}, 15000)

test('Bump each commit works', async () => {
  const repo = createTestRepo({ tagPrefix: '', bumpEachCommit: true }) // 0.0.0

  expect((await repo.runAction()).formattedVersion).toBe('0.0.0.0+0')
  repo.makeCommit('Initial Commit')
  expect((await repo.runAction()).formattedVersion).toBe('0.0.0.1+0')
  repo.makeCommit('Second Commit')
  expect((await repo.runAction()).formattedVersion).toBe('0.0.0.2+0')
  repo.makeCommit('Third Commit')
  expect((await repo.runAction()).formattedVersion).toBe('0.0.0.3+0')
  repo.makeCommit('Fourth Commit (MINOR)')
  expect((await repo.runAction()).formattedVersion).toBe('0.0.1.0+0')
  repo.makeCommit('Fifth Commit')
  expect((await repo.runAction()).formattedVersion).toBe('0.0.1.1+0')
  repo.makeCommit('Sixth Commit (MAJOR)')
  expect((await repo.runAction()).formattedVersion).toBe('1.0.0.0+0')
  repo.makeCommit('Seventh Commit')
  expect((await repo.runAction()).formattedVersion).toBe('1.0.0.1+0')
}, 15000)
...
test('Bump each commit picks up tags', async () => {
  ...
  expect((await repo.runAction()).formattedVersion).toBe('0.0.0.2+0')
  repo.makeCommit('Third Commit')
  repo.exec('git tag 3.0.0.0')
  expect((await repo.runAction()).formattedVersion).toBe('3.0.0.0+0')
  repo.makeCommit('Fourth Commit')
  expect((await repo.runAction()).formattedVersion).toBe('3.0.0.1+0')
}, 15000)
...
test('Increment not affected by matching tag', async () => {
  ...
  repo.makeCommit('Second Commit') // 0.0.1+1
  repo.exec('git tag 0.0.0.1')
  expect((await repo.runAction()).formattedVersion).toBe('0.0.0.1+1')
}, 15000)
...
test('Tag prefix can include forward slash', async () => {
  ...
  repo.makeCommit('Initial Commit')
  repo.exec('git tag version/1.0.2.3')
  const result = await repo.runAction()

  expect(result.formattedVersion).toBe('1.0.2.3+0')
}, 15000)
...
test('Tags immediately before merge are detected', async () => {
  ...
  expect(result.versionTag).toBe('v2.0.0.1')
}, 15000)
...
test('Correct tag is detected when versions pass 10s place', async () => {
  ...
  repo.makeCommit('Commit 1')
  repo.exec('git checkout -b feature/branch1')
  repo.exec('git tag v10.0.15.0')
  repo.makeCommit('Commit 2')
  repo.exec('git checkout master')
  repo.exec('git merge feature/branch1')
  repo.exec('git checkout -b feature/branch2')
  repo.makeCommit('Commit 3')
  repo.exec('git tag v10.0.7.0')
  repo.makeCommit('Commit 4')
  repo.exec('git checkout master')
  repo.makeCommit('Commit 5')
  repo.exec('git merge feature/branch2')
  ...
  expect(result.versionTag).toBe('v10.0.15.1')
}, 15000)
...
test('Tags on unmerged branches are not considered', async () => {
  ...
  expect(result.versionTag).toBe('v1.0.0.1')
}, 15000)
...
test('Can use branches instead of tags', async () => {
  ...
  repo.makeCommit('Initial Commit')
  repo.makeCommit('Commit 1')
  repo.exec('git checkout -b release/1.0.0.0')
  repo.makeCommit('Commit 2')
  repo.exec('git checkout master')
  repo.exec('git merge release/1.0.0.0')
  repo.makeCommit('Commit 3')
  const result = await repo.runAction()
